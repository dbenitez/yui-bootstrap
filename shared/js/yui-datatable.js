...
_initTableEl : function(elContainer) {
    if(elContainer) {
        ...
        this._elTable.className = "table table-condensed table-striped table-bordered"
...
showTableMessage : function(sHTML, sClassName) {
    ...
    elCell.firstChild.className = 'alert';
...
showCellEditorBtns : function(elContainer) {
    ...
    Dom.addClass(elSaveBtn,'btn');
    Dom.addClass(elSaveBtn,'btn-primary');
...
_initTableEl : function() {
    ...
    if(this._elHdContainer) {
        ...
        Dom.addClass(this._elHdTable,'table table-condensed table-bordered')
        // Set up mouseover/mouseout events via mouseenter/mouseleave delegation
...
renderBtns : function() {
    ...
    // Save button
    var elSaveBtn = elBtnsDiv.appendChild(document.createElement("button"));
    elSaveBtn.className = DT.CLASS_DEFAULT + " btn btn-primary";
...
renderForm : function() {
    if(lang.isArray(this.checkboxOptions)) {
        ...
        var sHTML = "";
        var tpl = '<label class="checkbox"><input type="checkbox" id="{id}" value="{value}"> {label}</label>';
        for(j=0,len=this.checkboxOptions.length; j<len; j++) {
            ...
            sHTML += lang.substitute(tpl,{
            	"id":this.getId() + "-chk" + j,
            	"value": checkboxValue,
            	"label": lang.isValue(checkboxOption.label) ? checkboxOption.label : checkboxOption
            })
        }
        this.getContainerEl().innerHTML = sHTML;
        // Store the reference to the checkbox elements
        this.checkboxes = this.getContainerEl().getElementsByTagName('INPUT');
        if(this.disableBtns) {
...
renderForm : function() {
    if(lang.isArray(this.radioOptions)) {
        ...
        var sHTML = "";
        var tpl = '<label class="radio"><input type="radio" id="{id}" name="{name}" value="{value}"> {label}</label>';
        for(var i=0, len=this.radioOptions.length; i<len; i++) {
        	radioOption = this.radioOptions[i];
            radioValue = lang.isValue(radioOption.value) ?
                    radioOption.value : radioOption;
            sHTML += lang.substitute(tpl,{
            	...
            	"name": this.getId(),
            	"value": radioValue,
            	"label": (lang.isValue(radioOption.label)) ? radioOption.label : radioOption
            })
        }
        this.getContainerEl().innerHTML = sHTML;
        // Store the reference to the checkbox elements
        this.radios = this.getContainerEl().getElementsByTagName('INPUT');
 ...
 */
move : function() {
    this.textarea.style.width = (this.getTdEl().offsetWidth - 15) + "px";
 ...
 */
move : function() {
    this.textbox.style.width = (this.getTdEl().offsetWidth-15) + "px";
