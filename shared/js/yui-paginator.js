...
lang.augmentObject(Paginator, {
     ...
     * @type string
     */
    TEMPLATE_DEFAULT : "<div class=\"span12\"><div class=\"pagination pagination-centered\"><ul>{FirstPageLink} {PreviousPageLink} {PageLinks} {NextPageLink} {LastPageLink}</ul></div></div>",

    TEMPLATE_DATATABLE: [
                         '<div class="span6">{CurrentPageReport}</div>',
                         '<div class="span6"><div class=\"pagination pagination-right\"><ul>{FirstPageLink} {PreviousPageLink} {PageLinks} {NextPageLink} {LastPageLink}</ul></div></div>'
                         ].join(''),
     TEMPLATE_DATATABLE_ROWS_PER_PAGE: [
                          '<div class="span5">{CurrentPageReport}</div>',
                          '<div class="span5"><div class=\"pagination\"><ul>{FirstPageLink} {PreviousPageLink} {PageLinks} {NextPageLink} {LastPageLink}</ul></div></div>',
                          '<div class="span2">{RowsPerPageDropdown}</div>'
                          ].join(''),
    ...
    TEMPLATE_ROWS_PER_PAGE : [
                              "<div class=\"span10\"><div class=\"pagination pagination-centered\"><ul>{FirstPageLink} {PreviousPageLink} {PageLinks} {NextPageLink} {LastPageLink}</ul></div></div>" +
                              "<div class=\"span2\" style=\"text-align: right;\"> {RowsPerPageDropdown} </div>"
                              ].join(''),
    ...
    renderUIComponent : function (marker, id_base) {
        ...
        if (isFunction(UIComp)) {
            ...
            if (isFunction(comp.render)) {
            	var node = comp.render(id_base);
            	if(lang.isArray(node) && !node.options){
            		for(var i = node.length; i > 0 ; i--) {
            			Dom.insertAfter(node[i-1],marker);
            		}
            		par.removeChild(marker);
            	} else {
            		par.replaceChild(node,marker);
            	}
            }
        }
...
Paginator.ui.CurrentPageReport.init = function (p) {
    ...
    p.setAttributeConfig('pageReportTemplate', {
        value : 'Showing {startRecord} to {endRecord} of {totalRecords} entries',
        validator : l.isString
    });
...
Paginator.ui.PageLinks.prototype = {
    ...
    update : function (e) {
        ...
        if (this.current !== currentPage || !currentPage || e.rebuild) {
            ...
            linkTemplate = '<li class="{liClass}"><a href="#" class="{class}" page="{page}" title="{title}">{label}</a></li>';
            //spanTemplate = '<span class="{class}">{label}</span>';
            for (i = start; i <= end; ++i) {
                    content += l.substitute(linkTemplate, {
                        'class' : p.get('pageLinkClass'),
                        'page'  : i,
                        'liClass': (i === currentPage) ? 'active' : '',
            ...
            if(content.length == 0) {
            	node.innerHTML = l.substitute(linkTemplate, {
                    'class' : p.get('pageLinkClass'),
                    'page'  : 0,
                    'liClass': 'disabled',
                    'label' : '0',
            ...
            if(this.container && this.container.length) this.parent = this.container[0].parentNode;
            this.container = [];
            for(var i = 0; i < nodes.length; i++){
            	this.container.push(nodes[i]);
            }
            if(this.parent) {
            	var pages = Dom.getElementsByClassName('yui-pg-page','a',this.parent);
            	if(pages.length) {
            		var marker = Dom.getPreviousSibling(pages[0].parentNode);
            		for(var i = 0; i < pages.length; i++) {this.parent.removeChild(pages[i].parentNode)};
            		YAHOO.util.Event.on(this.container,'click',this.onClick,this,true);
            		for(var i = this.container.length; i > 0 ; i--){Dom.insertAfter(this.container[i-1],marker)};
            	}
            }
        }
    },
...
Paginator.ui.FirstPageLink.prototype = {
    ...
    render : function (id_base) {
        ...
        this.current = document.createElement('li');
        this.link     = document.createElement('a');
        this.current.appendChild(this.link);
        //this.span     = document.createElement('span');
        ...
        if(p.getCurrentPage() > 1) {
        	Dom.removeClass(this.current,'disabled');
        } else {
        	Dom.addClass(this.current,'disabled');
        }
        return this.current;
    },
...
Paginator.ui.LastPageLink.prototype = {
    ...
    render : function (id_base) {
        ...
        this.current.appendChild(this.link);
        //this.span = document.createElement('span');
        this.na   = document.createElement('li');
...
Paginator.ui.RowsPerPageDropdown.init = function (p) {
    ...
    p.setAttributeConfig('rowsPerPageDropdownClass', {
        value : 'yui-pg-rpp-options span12',
        validator : l.isString
    });
};
